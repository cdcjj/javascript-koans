...
describe("About Applying What We Have Learnt", function() {
  ...
  it("given I'm allergic to nuts and hate mushrooms, it should find a pizza I can eat (functional)", function () {
      ...
      var filtered = _(products).filter(function(el) {return el.containsNuts === false});

      var isMushroom = function(x) {return x === "mushrooms"};

      for (var j = 0; j < filtered.length; j++){
        if (_(filtered[j].ingredients).any(isMushroom) !== true) {
          productsICanEat.push(filtered[j]);
        }
      }
  ...
  it("should add all the natural numbers below 1000 that are multiples of 3 or 5 (functional)", function () {
    var sum = _.range(0, 1000, 1)
                 .filter(function(x) {return x % 3 === 0 || x % 5 === 0;})
                 .reduce(function(memo, y) {return memo + y;}, 0);    /* try chaining range() and reduce() */
